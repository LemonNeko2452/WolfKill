...
object GameRestart : SimpleCommand(
    WolfKill,
    primaryName = "game-restart",
    secondaryNames = CommandConfig.GameRestart,
    description = "重新创建游戏房间"
) {
    ...
    suspend fun CommandSenderOnMessage<*>.handle() {
        ...
        for (room in WolfKillRoom.rooms) {
            if (room.room == thisGroup) flagy = false
        }
        ...
        if (flagy) {
            sendMessage("没有正在进行的游戏，请输入 game-start 创建游戏")
            return
        }
        if (flagx) {
